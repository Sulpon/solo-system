...
import type { ChecklistItem, ChecklistMode, ChecklistTier } from "../../_lib/types/quest";
import type { ChecklistTemplate } from "../../_lib/types/checklist-template";

// The one real checklist-editing UI (mode switcher, Minimum/Full tier lists
// with drag-and-drop, Fixed-mode templates) - shared by QuestChecklistTab.tsx
// (the Quest detail view's "Checklist" tab, which persists every edit
// immediately via onUpdate) and QuestForm.tsx (the create/edit modal, which
// only holds edits in local form state until "Save Quest"). Both callers
// look identical from here: a `{ checklistMode?, checklist?,
// checklistTemplateId? }` patch object, the exact same shape
// onUpdateChecklist/updateChecklist already use elsewhere - a caller just
// decides whether that patch writes straight to the real Quest or into its
// own local form state. This file owns no persistence of its own.
export type ChecklistPatch = Readonly<{ checklistMode?: ChecklistMode; checklist?: ReadonlyArray<ChecklistItem>; checklistTemplateId?: string | null }>;

type ChecklistEditorProps = Readonly<{
  mode: ChecklistMode;
  checklist: ReadonlyArray<ChecklistItem>;
  checklistTemplateId?: string | null;
  onUpdate: (patch: ChecklistPatch) => void;
}>;
...
export default function ChecklistEditor({ mode, checklist, checklistTemplateId, onUpdate }: ChecklistEditorProps) {
  ...
  const linkedTemplate = checklistTemplateId ? templates.find((template) => template.id === checklistTemplateId) : undefined;

  function setMode(nextMode: ChecklistMode) {
    onUpdate({ checklistMode: nextMode });
  }

  function toggleItem(itemId: string) {
    onUpdate({ checklist: checklist.map((item) => (item.id === itemId ? { ...item, completed: !item.completed } : item)) });
  }

  function editItemTitle(itemId: string, title: string) {
    onUpdate({ checklist: checklist.map((item) => (item.id === itemId ? { ...item, title } : item)) });
  }

  function deleteItem(itemId: string) {
    onUpdate({ checklist: checklist.filter((item) => item.id !== itemId) });
  }

  function addItem(tier: ChecklistTier, title: string) {
    onUpdate({ checklist: [...checklist, createChecklistItem(title, tier)] });
  }

  function moveItemToTier(itemId: string, tier: ChecklistTier) {
    onUpdate({ checklist: moveChecklistItem(checklist, itemId, tier, Number.POSITIVE_INFINITY) });
  }
  ...
  function applyTemplate(templateId: string) {
    ...
    onUpdate({ checklist: copiedItems, checklistTemplateId: template.id });
  ...
  function saveAsNewTemplate() {
    ...
    onUpdate({ checklistTemplateId: template.id });
    setNewTemplateName("");
  }
  ...
  function updateLinkedTemplate() {
    ...
    setTemplates((current) =>
      current.map((template) => (template.id === checklistTemplateId ? { ...template, items: checklist.map((item) => ({ ...item, completed: false })), updatedAt: now } : template)),
    );
  }
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    onUpdate({ checklist: moveChecklistItem(checklist, itemId, targetTier, targetIndex) });
  }
